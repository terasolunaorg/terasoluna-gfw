 ...
 * In case of {@code SystemException}, the value of {@link SystemException#getCode()} is considered
 * as exception code. <br>
 * If the value of {@code SystemException#getCode()} is null, exception code is decided based on
 * specification similar to exceptions<br>
 * other than {@code SystemException}<br>
 * In case of exception other than {@code SystemException}, exception code is decided as defined in
 * mappings in {@link #exceptionMappings}<br>
 * If there is no definition in {@link #exceptionMappings}, then {@link #defaultExceptionCode} is
 * taken as exception code.
 * </p>
 */
public class SimpleMappingExceptionCodeResolver implements ExceptionCodeResolver {
     ...
     * <li>It is possible to specify a part of class name in {@code key}. For example, by setting
     * {@code NotFound} in<br>
     * {@code key}, {@code java.io.FileNotFoundException} as well as
     * {@code javax.naming.NameNotFoundException} are covered.
     * <li>If multiple {@code key} are set, it will be applicable in the order of addition to the
     * {@code Map}</li>
     * </ul>
     ...
     * <p>
     * Exception code that must be applied when it could not be resolved based on rules, must be
     * specified.<br>
     ...
     * Returns default exception code if exception code could not be determined based on rules. If
