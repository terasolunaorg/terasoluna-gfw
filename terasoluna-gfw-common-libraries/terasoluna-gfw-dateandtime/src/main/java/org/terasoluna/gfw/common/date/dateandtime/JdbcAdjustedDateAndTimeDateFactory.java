 ...
 * @since 5.0.0
 */
public class JdbcAdjustedDateAndTimeDateFactory extends AbstractDateAndTimeDateFactory implements InitializingBean {
	private static final Logger logger = LoggerFactory.getLogger(JdbcAdjustedDateAndTimeDateFactory.class);
	...
	public LocalDateTime newLocalDateTime() {
		...
		return LocalDateTime.now().plus(adjustedValue, ChronoUnit.MILLIS);
	}

	/**
	 * Returns the adjusted current system {@link java.time.OffsetDateTime}
	 * <P>
	 * Adjustment value can come from DB or cached adjustment value can <br>
	 * be used to adjust the current system DateTime <br>
	 * configuration.<br>
	 *
	 * @return OffsetDateTime adjusted current DateTime
	 */
	@Override
	public OffsetDateTime newOffsetDateTime() {
		...
		return OffsetDateTime.now().plus(adjustedValue, ChronoUnit.MILLIS);
	}

	/**
	 * Returns the adjusted current system {@link java.time.ZonedDateTime}
	 * <P>
	 * Adjustment value can come from DB or cached adjustment value can <br>
	 * be used to adjust the current system DateTime <br>
	 * configuration.<br>
	 *
	 * @return ZonedDateTime adjusted current DateTime
	 */
	@Override
	public ZonedDateTime newZonedDateTime() {
		...
		return ZonedDateTime.now().plus(adjustedValue, ChronoUnit.MILLIS);
	}
	 ...
	 * 
	 * @param useCache Attribute that specifies whether to cache the adjustment
	 *                 value
	 */
	 ...
	 * 
	 * @return boolean Attribute that specifies whether to cache the adjustment
	 *         value
	 */
