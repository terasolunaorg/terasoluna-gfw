...
/**
 * {@code HandlerInterceptor} implementation class that introduces TransactionTokenCheck
 * functionality for each incoming HTTP Request.
 */
public class TransactionTokenInterceptor implements HandlerInterceptor {
    ...
    public static final String TOKEN_CONTEXT_REQUEST_ATTRIBUTE_NAME =
            TransactionTokenInterceptor.class.getName() + ".TOKEN_CONTEXT";
    ...
    public static final String NEXT_TOKEN_REQUEST_ATTRIBUTE_NAME =
            TransactionTokenInterceptor.class.getName() + ".NEXT_TOKEN";
     ...
     * <p>
     * sizePerTokenName is the number of tokenKeys that are allowed to be generated per
     * tokenName.<br>
     ...
     * {@link HttpSessionTransactionTokenStore} is used to store transaction tokens. Default size
     * per tokenName(namespace) is used.
     * </p>
     ...
     * {@link TokenStringGenerator} is used to generate token string.<br>
     * {@link TransactionTokenInfoStore} is used to store {@link TransactionTokenInfo}.<br>
     * {@link HttpSessionTransactionTokenStore} is used to store transaction tokens. The size per
     * tokenName(namespace) is given.
     * </p>
    ...
    /**
     * Constructor that takes tokenStringGenerator, transactionTokenInfoStore and
     * transactionTokenStore as parameters
    ...
    public TransactionTokenInterceptor(TokenStringGenerator generator,
            TransactionTokenInfoStore tokenInfoStore, TransactionTokenStore tokenStore) {
     ...
     * <p>
     * If token check passes, sets context information of the token in the
     * "TransactionTokenInterceptor.TOKEN_CONTEXT" request attribute and returns true.
     * <p>
     * This method expects the handler argument to be an instance of <code>HandlerMethod</code>
     * class. If the handler is not an instance of <code>HandlerMethod</code> class, the method
     * returns true without executing the validation. <br>
    ...
    public boolean preHandle(final HttpServletRequest request, final HttpServletResponse response,
        ...
        TransactionTokenInfo tokenInfo = tokenInfoStore.getTransactionTokenInfo(handlerMethod);
        ...
        if (tokenInfo.getTransactionTokenType() == TransactionTokenType.BEGIN) {
            // This logic is added later to remove existing token sent in the request in case of
            // Transaction BEGIN
            // When transactions BEGIN, the transaction token sent from the request are usually
            // those which are generated
            // due to input errors. (Since method with BEGIN may be called in spite of input errors,
        ...
        TransactionTokenContextImpl tokenContext =
                new TransactionTokenContextImpl(tokenInfo, receivedToken);
     ...
     * <p>
     * Returns false if the instance of receivedToken is not valid or token namespace of the target
     * method and passed token does not match. <br>
     * Returns true if <br>
     ...
     * <p>
     * Take note that once a token is fetched from tokenStore, its value is cleared in tokenStore.
     * Hence this method can return true only once. All further invocations to this method for the
     * same receivedToken will return false.
     * @param receivedToken
     * @param tokenStore
     * @param tokenInfo
    ...
    boolean validateToken(final TransactionToken receivedToken,
            final TransactionTokenStore tokenStore, final TransactionTokenInfo tokenInfo) {

        if (receivedToken.valid()
                && receivedToken.getTokenName().equals(tokenInfo.getTokenName())) {

            String storedToken = tokenStore.getAndClear(receivedToken);
            if (storedToken != null && storedToken.equals(receivedToken.getTokenValue())) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Based on context information from the request attribute named
     * <code>TransactionTokenInterceptor.TOKEN_CONTEXT</code>, creates, updates or keeps the token
     * stored with the request attribute <code>TransactionTokenInterceptor.NEXT_TOKEN</code> and
     * also in the <code>TransactionTokenStore</code> or removes the token from
     ...
     * @param modelAndView the <code>ModelAndView</code> that the handler returned (can also be
     *        <code>null</code>)
    ...
    public void postHandle(final HttpServletRequest request, final HttpServletResponse response,
        ...
        switch (tokenContext.getReserveCommand()) {
            case CREATE_TOKEN:
                createToken(request, request.getSession(true), tokenContext.getTokenInfo(),
                        generator, tokenStore);
                break;
            case UPDATE_TOKEN:
                updateToken(request, request.getSession(true), tokenContext.getReceivedToken(),
                        tokenContext.getTokenInfo(), generator, tokenStore);
                break;
            case REMOVE_TOKEN:
                removeToken(tokenContext.getReceivedToken());
                break;
            case KEEP_TOKEN:
                keepToken(request, tokenContext.getReceivedToken(), tokenContext.getTokenInfo(),
                        tokenStore);
                break;
            default:
                // noop
     ...
     * <code>TransactionTokenInterceptor.TOKEN_CONTEXT</code> Arguments <code>response</code> and
     * <code>handler</code> are not used in this implementation
    ...
    /**
     * Updates the value of existing token in <code>TransactionTokenStore</code> as well as in the
     * request
     * <p>
    ...
    void updateToken(HttpServletRequest request, HttpSession session,
        ...
        TransactionToken nextToken = new TransactionToken(tokenInfo.getTokenName(),
                receivedToken.getTokenKey(), generator.generate(session.getId()));
     ...
     * <p>
     * Generated <code>TransactionToken</code> instance is stored in
     * <code>TransactionTokenStore</code> and also set to request attribute
     * <code>TransactionTokenInterceptor.NEXT_TOKEN</code>.
     * <p>
     * @param request
     * @param session
    ...
    /**
     * Set the receivedToken to the attributes named
     * <code>TransactionTokenInterceptor.NEXT_TOKEN</code> of request. And receivedToken stored in
     * <code>TransactionTokenStore</code> without updates.
     * @param request current HTTP request
